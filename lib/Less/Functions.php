...
class Functions{
    ...
    static public function number($n){
        ...
        if ($n instanceof \Less\Tree\Dimension) {
            ...
            throw new \Less\Exception\Compiler("color functions take numbers as parameters");
        }
    }

    static public function scaled($n, $size = 256 ){
        if( $n instanceof \Less\Tree\Dimension && $n->unit->is('%') ){
            return (float)$n->value * $size / 100;
        } else {
            return \Less\Functions::number($n);
        }
    }
    ...
    public function rgba($r, $g, $b, $a){
        $rgb = array($r, $g, $b);
        $rgb = array_map(array('\Less\Functions','scaled'),$rgb);

        $a = self::number($a);
        return new \Less\Tree\Color($rgb, $a);
    }
    ...
    public function hsva($h, $s, $v, $a) {
        $h = ((\Less\Functions::number($h) % 360) / 360 ) * 360;
        $s = \Less\Functions::number($s);
        $v = \Less\Functions::number($v);
        $a = \Less\Functions::number($a);
    ...
    public function hue($color){
        $c = $color->toHSL();
        return new \Less\Tree\Dimension(\Less\Parser::round($c['h']));
    }

    public function saturation($color){
        $c = $color->toHSL();
        return new \Less\Tree\Dimension(\Less\Parser::round($c['s'] * 100), '%');
    }

    public function lightness($color){
        $c = $color->toHSL();
        return new \Less\Tree\Dimension(\Less\Parser::round($c['l'] * 100), '%');
    }

    public function hsvhue( $color ){
        $hsv = $color->toHSV();
        return new \Less\Tree\Dimension( \Less\Parser::round($hsv['h']) );
    }


    public function hsvsaturation( $color ){
        $hsv = $color->toHSV();
        return new \Less\Tree\Dimension( \Less\Parser::round($hsv['s'] * 100), '%' );
    }

    public function hsvvalue( $color ){
        $hsv = $color->toHSV();
        return new \Less\Tree\Dimension( \Less\Parser::round($hsv['v'] * 100), '%' );
    }

    public function red($color) {
        return new \Less\Tree\Dimension( $color->rgb[0] );
    }

    public function green($color) {
        return new \Less\Tree\Dimension( $color->rgb[1] );
    }

    public function blue($color) {
        return new \Less\Tree\Dimension( $color->rgb[2] );
    }

    public function alpha($color){
        $c = $color->toHSL();
        return new \Less\Tree\Dimension($c['a']);
    }

    public function luma ($color) {
        return new \Less\Tree\Dimension(\Less\Parser::round( $color->luma() * $color->alpha * 100), '%');
    }
    ...
    public function darken($color, $amount){
        ...
        if( $color instanceof \Less\Tree\Color ){
            $hsl = $color->toHSL();
            $hsl['l'] -= $amount->value / 100;
            $hsl['l'] = self::clamp($hsl['l']);
        ...
        \Less\Functions::Expected('color',$color);
    }
    ...
    public function mix($color1, $color2, $weight = null){
        if (!$weight) {
            $weight = new \Less\Tree\Dimension('50', '%');
        }
        ...
        return new \Less\Tree\Color($rgb, $alpha);
    }

    public function greyscale($color){
        return $this->desaturate($color, new \Less\Tree\Dimension(100));
    }
    ...
    public function contrast( $color, $dark = false, $light = false, $threshold = false) {
        ...
        if( $threshold === false ){
            ...
            $threshold = \Less\Functions::number($threshold);
        }
    ...
    public function e ($str){
        return new \Less\Tree\Anonymous($str instanceof \Less\Tree\JavaScript ? $str->evaluated : $str);
    }
    ...
    public function escape ($str){
        ...
        return new \Less\Tree\Anonymous(strtr(rawurlencode($str->value), $revert));
    }
    ...
    public function unit($val, $unit = null ){
        if( !($val instanceof \Less\Tree\Dimension) ){
            throw new \Less\Exception\Compiler('The first argument to unit must be a number' . ($val instanceof \Less\Tree\Operation ? '. Have you forgotten parenthesis?' : '.') );
        }
        return new \Less\Tree\Dimension($val->value, $unit ? $unit->toCSS() : "");
    }
    ...
    public function round($n, $f = false) {
        ...
        return $this->_math('\Less\Parser::round',null, $n, $fraction);
    }

    public function pi(){
        return new \Less\Tree\Dimension(M_PI);
    }
    ...
    public function pow($x, $y) {
        if( is_numeric($x) && is_numeric($y) ){
            $x = new \Less\Tree\Dimension($x);
            $y = new \Less\Tree\Dimension($y);
        }elseif( !($x instanceof \Less\Tree\Dimension) || !($y instanceof \Less\Tree\Dimension) ){
            throw new \Less\Exception\Compiler('Arguments must be numbers');
        }

        return new \Less\Tree\Dimension( pow($x->value, $y->value), $x->unit );
    }
    ...
    private function _math() {
        ...
        if ($args[0] instanceof \Less\Tree\Dimension) {
            ...
            return new \Less\Tree\Dimension( call_user_func_array($fn, $args), $unit);
        } else if (is_numeric($args[0])) {
            return call_user_func_array($fn,$args);
        } else {
            throw new \Less\Exception\Compiler("math functions take numbers as parameters");
        }
    }
    ...
    function _minmax( $isMin, $args ){
        ...
        switch( count($args) ){
            case 0: throw new \Less\Exception\Compiler( 'one or more arguments required');
        ...
        for( $i = 0; $i < count($args); $i++ ){
            $current = $args[$i];
            if( !($current instanceof \Less\Tree\Dimension) ){
                $order[] = $current;
                continue;
            }
        ...
        $args = implode( \Less\Environment::$comma_space, $order);

        return new \Less\Tree\Anonymous( ($isMin ? 'min' : 'max') . '(' . $args . ')');
    }
    ...
    public function argb($color) {
        return new \Less\Tree\Anonymous($color->toARGB());
    }

    public function percentage($n) {
        return new \Less\Tree\Dimension($n->value * 100, '%');
    }

    public function color($n) {

        if( $n instanceof \Less\Tree\Quoted ){
            $colorCandidate = $n->value;
            $returnColor = \Less\Tree\Color::fromKeyword($colorCandidate);
            if( $returnColor ){
                return $returnColor;
            }
            if( preg_match('/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})/',$colorCandidate) ){
                return new \Less\Tree\Color(substr($colorCandidate, 1));
            }
            throw new \Less\Exception\Compiler("argument must be a color keyword or 3/6 digit hex e.g. #FFF");
        } else {
            throw new \Less\Exception\Compiler("argument must be a string");
        }
    }


    public function iscolor($n) {
        return $this->_isa($n, '\Less\Tree\Color');
    }

    public function isnumber($n) {
        return $this->_isa($n, '\Less\Tree\Dimension');
    }

    public function isstring($n) {
        return $this->_isa($n, '\Less\Tree\Quoted');
    }

    public function iskeyword($n) {
        return $this->_isa($n, '\Less\Tree\Keyword');
    }

    public function isurl($n) {
        return $this->_isa($n, '\Less\Tree\Url');
    }
    ...
    public function isunit( $n, $unit ){
        return ($n instanceof \Less\Tree\Dimension) && $n->unit->is( ( property_exists($unit,'value') ? $unit->value : $unit) ) ? new \Less\Tree\Keyword('true') : new \Less\Tree\Keyword('false');
    }

    private function _isa($n, $type) {
        return is_a($n, $type) ? new \Less\Tree\Keyword('true') : new \Less\Tree\Keyword('false');
    }
    ...
    function length($values){
        ...
        return new \Less\Tree\Dimension($n);
    }
    ...
    function datauri($mimetypeNode, $filePathNode = null ) {
        ...
        if( \Less\Environment::isPathRelative($filePath) ){
            ...
            if( !empty($temp) ){
                $filePath = \Less\Environment::normalizePath(rtrim($temp,'/').'/'.$filePath);
            }
        ...
        if( $fileSizeInKB >= $DATA_URI_MAX_KB ){
            $url = new \Less\Tree\Url( ($filePathNode ? $filePathNode : $mimetypeNode), $this->currentFileInfo);
        ...
        return new \Less\Tree\Url( new \Less\Tree\Anonymous($filePath) );
    }
    ...
    function svggradient( $direction ){
        ...
        if( count($arguments) < 3 ){
            throw new \Less\Exception\Compiler( $throw_message );
        }
        ...
        $useBase64 = true;
        $renderEnv = new \Less\Environment();
        ...
        switch( $directionValue ){
            ...
            case "ellipse at center":
                ...
                break;
            default:
                throw new \Less\Exception\Compiler( "svg-gradient direction must be 'to bottom', 'to right', 'to bottom right', 'to top right' or 'ellipse at center'" );
        }
        ...
        for( $i = 0; $i < count($stops); $i++ ){
            ...
            if( !($color instanceof \Less\Tree\Color) || (!(($i === 0 || $i+1 === count($stops)) && $position === null) && !($position instanceof \Less\Tree\Dimension)) ){
                throw new \Less\Exception\Compiler( $throw_message );
            }
            if( $position ){
        ...
        return new \Less\Tree\URL( new \Less\Tree\Anonymous( $returner ) );
    }
    ...
    private static function Expected( $type, $arg ){
        ...
        throw new \Less\Exception\Compiler($message);
