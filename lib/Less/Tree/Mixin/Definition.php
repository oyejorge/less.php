...
class Definition extends \Less\Tree\Ruleset{
    public $name;
    public $selectors;
    public $params;
    public $arity;
    ...
    public function __construct($name, $params, $rules, $condition, $variadic = false){
        $this->name = $name;
        $this->selectors = array(new \Less\Tree\Selector(array( new \Less\Tree\Element(null, $name))));
    ...
    public function compileParams($env, $mixinEnv, $args = array() , &$evaldArguments = array() ){
        $frame = new \Less\Tree\Ruleset(null, array());
        $varargs;
        $params = $this->params;
        $val;
        $name;
            ...
            if( $arg && $arg['name'] ){
                ...
                foreach($params as $j => $param){
                    if( !isset($evaldArguments[$j]) && $name === $params[$j]['name']) {
                        ...
                        array_unshift($frame->rules, new \Less\Tree\Rule( $name, $arg['value']->compile($env) ) );
                        $isNamedFound = true;
                        break;
                    }
                }
                if ($isNamedFound) {
                    ...
                    throw new \Less\Exception\Compiler("Named argument for " . $this->name .' '.$args[$i]['name'] . ' not found');
                }
            }
        }
        ...
        foreach($params as $i => $param){
            ...
            if (isset($param['name']) && $param['name']) {
                ...
                if( isset($param['variadic']) && $args ){
                    ...
                    $expression = new \Less\Tree\Expression($varargs);
                    array_unshift($frame->rules, new \Less\Tree\Rule($param['name'], $expression->compile($env)));
                }else{
                    ...
                    if ($val) {
                        ...
                        throw new \Less\Exception\Compiler("Wrong number of arguments for " . $this->name . " (" . count($args) . ' for ' . $this->arity . ")");
                    }

                    array_unshift($frame->rules, new \Less\Tree\Rule($param['name'], $val));
    ...
    public function compile($env, $args = NULL, $important = NULL) {
        ...
        $ex = new \Less\Tree\Expression($_arguments);
        array_unshift($frame->rules, new \Less\Tree\Rule('@arguments', $ex->compile($env)));


        $rules = array_slice($this->rules,0);

        $ruleset = new \Less\Tree\Ruleset(null, $rules);
