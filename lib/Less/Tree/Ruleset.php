...
class Ruleset extends \Less\Tree{
    ...
    public function setRulesetIndex(){
        $this->ruleset_id = \Less\Parser::$next_id++;
    ...
    public function compile($env){
        ...
        $ruleset = new \Less\Tree\Ruleset($selectors, $this->rules, $this->strictImports);
        $rules = array();
        ...
        for( $i = 0; $i < $ruleset_len; $i++ ){
            if( $ruleset->rules[$i] instanceof \Less\Tree\Mixin\Definition ){
        ...
        for($i=0; $i < $ruleset_len; $i++){
            ...
            if( $rule instanceof \Less\Tree\Mixin\Call ){
                $rules = $rule->compile($env);

                $temp = array();
                foreach($rules as $r){
                    if( ($r instanceof \Less\Tree\Rule) && $r->variable ){
    ...
    function makeImportant(){
        ...
        foreach($this->rules as $rule){
            if( $rule instanceof \Less\Tree\Rule || $rule instanceof \Less\Tree\Ruleset ){
        ...
        return new \Less\Tree\Ruleset($this->selectors, $important_rules, $this->strictImports );
    }
    ...
    public function variables(){
        ...
        if( !$this->_variables ){
            ...
            foreach( $this->rules as $r){
                if ($r instanceof \Less\Tree\Rule && $r->variable === true) {
                    ...
                    foreach( $rule->selectors as $ruleSelector ){
                        ...
                        if( $match ){
                            if( $selector->elements_len > $match ){
                                $this->lookups[$key] = array_merge($this->lookups[$key], $rule->find( new \Less\Tree\Selector(array_slice($selector->elements, $match)), $self, $env));
                            } else {
    ...
    public function genCSS( $env, &$strs ){
        ...
        if( !\Less\Environment::$compress && $env->tabLevel ){
        ...
        foreach($this->rules as $rule){
            ...
            if( ($class === '\Less\Tree\Media') || ($class === '\Less\Tree\Directive') || ($this->root && $class === '\Less\Tree\Comment') || ($class === '\Less\Tree\Ruleset' && $rule->rules) ){
                $rulesetNodes[] = $rule;
            }else{
                $ruleNodes[] = $rule;
            }
        ...
        if( !$this->root ){
            ...
            */
            ...
            for( $i = 0,$paths_len = count($this->paths); $i < $paths_len; $i++ ){
                $path = $this->paths[$i];
                \Less\Environment::$firstSelector = true;
                foreach($path as $p){
                    $p->genCSS($env, $strs );
                    \Less\Environment::$firstSelector = false;
                }
                if( $i + 1 < $paths_len ){
                    self::outputAdd( $strs, \Less\Environment::$compress ? ',' : (",\n" . $tabSetStr) );
                }
            }
        ...
        if( !$strs && !\Less\Environment::$compress && $this->firstRoot ){
            self::outputAdd( $strs, "\n" );
        }
                ...
                foreach($newSelectors as $sel){
                    ...
                    if( !$context ){
                        ...
                        if( $sel ){
                            ...
                            $sel[0]->elements[] = new \Less\Tree\Element($el->combinator, '', 0, $el->index, $el->currentFileInfo );
                        }
                        ...
                        foreach($context as $parentSel){
                            ...
                            if ( $parentSel ){
                                ...
                                $newJoinedSelector->elements[] = new \Less\Tree\Element( $el->combinator, $parentSel[0]->elements[0]->value, 0, $el->index, $el->currentFileInfo);
    ...
    function mergeElementsOnToSelectors( $elements, &$selectors){
        ...
        if( !$selectors ){
            $selectors[] = array( new \Less\Tree\Selector($elements) );
            return;
        }
        ...
        foreach( $selectors as &$sel){
            ...
            if( $sel ){
                ...
                $sel[] = new \Less\Tree\Selector( $elements );
            }
        }
    }
}
