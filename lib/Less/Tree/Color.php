...
class Color extends \Less\Tree{
    ...
    public function toCSS($env = null, $doNotCompress = false ){
        $compress = \Less\Environment::$compress && !$doNotCompress;
    ...
    public function operate($env, $op, $other) {
        ...
        if (! ($other instanceof \Less\Tree\Color)) {
            $other = $other->toColor();
        }

        for ($c = 0; $c < 3; $c++) {
            $result[$c] = \Less\Functions::operate($env, $op, $this->rgb[$c], $other->rgb[$c]);
        }
        return new \Less\Tree\Color($result, $this->alpha + $other->alpha);
    }

    public function toRGB(){
        $color = '';
        foreach($this->rgb as $i){
            $i = \Less\Parser::round($i);
            $i = ($i > 255 ? 255 : ($i < 0 ? 0 : $i));
            $i = dechex($i);
    ...
    public function toARGB(){
        $argb = array_merge( (array) \Less\Parser::round($this->alpha * 255), $this->rgb);
