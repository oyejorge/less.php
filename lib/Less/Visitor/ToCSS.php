...
class ToCSS extends \Less\VisitorReplacing{
    ...
    function checkPropertiesInRoot( $rules ){
        foreach($rules as $ruleNode){
            if( $ruleNode instanceof \Less\Tree\Rule && !$ruleNode->variable ){
                ...
                throw new \Less\Exception\Compiler($msg);
            }
        }
    }
        ...
        if( !$rulesetNode->root ){
            ...
            foreach($rulesetNode->paths as $p){
                if( $p[0]->elements[0]->combinator->value === ' ' ){
                    $p[0]->elements[0]->combinator = new \Less\Tree\Combinator('');
                }
        ...
        for( $i = count($rules)-1; $i >= 0 ; $i-- ){
            ...
            if( $rule instanceof \Less\Tree\Rule ){
                if( !isset($ruleCache[$rule->name]) ){
                    ...
                    if( $ruleList instanceof \Less\Tree\Rule ){
    ...
    function _mergeRules( &$rules ){
        ...
        for( $i = 0; $i < $rules_len; $i++ ){
            ...
            if( ($rule instanceof \Less\Tree\Rule) && $rule->merge ){
        ...
        foreach($groups as $parts){
            ...
            if( count($parts) > 1 ){
                ...
                $rule->value = new \Less\Tree\Value( $values );
            }
        }
    }
}
