...
namespace Less\Visitor;

class ProcessExtends extends \Less\Visitor{

    public $allExtendsStack;

    function run( $root ){
        $extendFinder = new \Less\Visitor\ExtendFinder();
    ...
    function doExtendChaining( $extendsList, $extendsListTarget, $iterationCount = 0){
            ...
            for( $targetExtendIndex = 0; $targetExtendIndex < count($extendsListTarget); $targetExtendIndex++ ){
                ...
                if( $matches ){
                    ...
                    foreach($extend->selfSelectors as $selfSelector ){
                        ...
                        $newExtend = new \Less\Tree\Extend( $targetExtend->selector, $targetExtend->option, 0);
        ...
        if( $extendsToAdd ){
            ...
            if( $iterationCount > 100) {
                ...
                throw new \Less\Exception\Parser("extend circular reference detected. One of the circular extends is currently:"+$selectorOne+":extend(" + $selectorTwo+")");
            }
    ...
    function isElementValuesEqual( $elementValue1, $elementValue2 ){
        ...
        if( $elementValue1 instanceof \Less\Tree\Attribute ){
        ...
        if( $elementValue1 instanceof \Less\Tree\Selector ){
            $elementValue2 = $elementValue2->value;
            if( !($elementValue2 instanceof \Less\Tree\Selector) || $elementValue1->elements_len !== $elementValue2->elements_len ){
                return false;
            }
    ...
    function extendSelector($matches, $selectorPath, $replacementSelector){
        ...
        for($matchIndex = 0, $matches_len = count($matches); $matchIndex < $matches_len; $matchIndex++ ){
            ...
            $firstElement = new \Less\Tree\Element(
            ...
            if( $currentSelectorPathIndex === $match['pathIndex'] && $matchIndex > 0 ){
                ...
                $path[] = new \Less\Tree\Selector( $newElements );
            }
